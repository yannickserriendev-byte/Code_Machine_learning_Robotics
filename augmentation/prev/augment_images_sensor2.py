"""
Augmentation utility for Sensor 2 images (grayscale, noise, rotation, masking).

- Augments each image with configurable number of rotations, color jitter, and circular masking.
- Adds Gaussian noise to each augmentation.
- Configurable: num_augmentations_per_image, FINAL_SIZE, IMAGE_SHIFT_RIGHT, num_threads.
...
base_dir = "/scratch/yserrrien/data aqcuisition/1/Data_Sensor_2/full_dataset"
...
def process_row(row: pd.Series, output_folder: str) -> list:
    ...
    for i in range(num_augmentations_per_image):
        ...
        # 5) Rotate (around center) — keep expand so we can recrop
