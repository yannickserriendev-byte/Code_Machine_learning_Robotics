...
MODEL_BASE_DIR = r"Y:\Master Thesis\Data_Sensor_2\full_dataset"
MODEL_PATH = os.path.join(MODEL_BASE_DIR, "model_ResNet18_Sensor_2_no_zip_a100_small", "checkpoint_epoch_34.pth")
BASE_DIR = r"Y:\Master Thesis\Data_Sensor_2\full_dataset"
LABELS_CSV = os.path.join(BASE_DIR, "1.augmented_labels_full_pipeline_ShapeWithNone_XYWithNan_based_on_Fz_indentorclassfix.csv")
DEVICE = torch.device("cpu")
MAX_POINTS = 100

dataset = IsochromaticDataset(
    images_folder=os.path.join(BASE_DIR, "1.augmented_images_full_pipeline"),
...
class CustomPreprocessor:
    ...
    def __call__(self, img_bgr):
        ...
        img = img_aug.resize((self.final_size, self.final_size))
        tensor = TF.to_tensor(img).repeat(3, 1, 1)
        tensor = TF.normalize(tensor, mean=[0.5]*3, std=[0.5]*3)
        return tensor, img_aug
...
ax_img_aug.set_title("Masked Input (grayscale)")
ax_img_aug.axis("off")
